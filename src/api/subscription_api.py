...
async def create_checkout_session(user_id: str):
    ...
    try:
        ...
        session = stripe.checkout.Session.create(
            ...
            line_items=[{
                'price_data': {
                    ...
                    'product_data': {
                        'name': 'AI DevOps Autopilot - Pro Plan',
                        'description': 'Monthly subscription with autonomous mode'
                    },
                    'recurring': {
                        'interval': 'month'
                    }
            ...
            mode='subscription',
            success_url=f"{os.getenv('APP_URL', 'http://localhost:8000')}/subscription/success?session_id={{CHECKOUT_SESSION_ID}}",
            cancel_url=f"{os.getenv('APP_URL', 'http://localhost:8000')}/subscription/cancel",
        ...
        print(f"[SUBSCRIPTION ERROR] create_checkout_session: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def stripe_webhook(request: Request):
    ...
    try:
        ...
        if event['type'] == 'checkout.session.completed':
            ...
            print(f"[STRIPE] Payment successful for {user_id}")
        ...
        print(f"[STRIPE ERROR] webhook: {e}")
        raise HTTPException(status_code=400, detail=str(e))
